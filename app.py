...
if uploaded_file is not None:
    ...
    # Post-processing Phase 2: Aggressive CLAHE for "HD" Contrast
    img_lab = cv2.cvtColor(img_uint8, cv2.COLOR_RGB2LAB)
    l, a, b = cv2.split(img_lab)
    clahe = cv2.createCLAHE(clipLimit=4.0, tileGridSize=(8, 8)) # Pushed limit to 4.0
    ...
    # Post-processing Phase 3: Aggressive Unsharp Masking for "HD" Textures
    # Pushed percent to 300 to hyper-sharpen edges
    hd_sharpened_img = enhanced_pil.filter(ImageFilter.UnsharpMask(radius=1.0, percent=300, threshold=2))
    ...
    with col2:
        st.image(hd_sharpened_img, caption="OrcaCV Enhanced (HD Profile)")
